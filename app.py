...
Generated by AI Fitness Optimizer (Multi-Model System)
...
Height: {height} cm | Weight: {weight} kg | Body Fat: {body_fat}%
Training Experience: {training_exp} years

Current Strength Levels:
- Bench Press: {bench_press} kg
- Squat: {squat} kg
- Deadlift: {deadlift} kg
- Total Lifted: {total_lifted} kg
- Strength-to-Weight Ratio: {strength_to_weight:.2f}

Activity Level:
- Daily Steps: {daily_steps}
- BMI: {bmi:.1f}

Goals: {', '.join([g for g, v in [('Muscle Gain', goal_muscle_gain), ('Fat Loss', goal_fat_loss), ('Strength', goal_strength), ('General Fitness', goal_general_fitness)] if v])}

================================================================================
AI-GENERATED RECOMMENDATIONS (6 Models)
================================================================================

TRAINING SPLIT: {split_info['name']}
Confidence: {split_confidence:.1f}%
Description: {split_info['description']}
Sample Schedule: {split_info['example']}

TRAINING PARAMETERS:
• Training Days: {int(predictions['training_days']['value'])} days per week
• Weekly Volume: {int(predictions['weekly_volume']['value'])} sets per muscle group
• Training Intensity: {int(predictions['intensity']['value'])}% of 1RM
• Cardio: {int(predictions['cardio_minutes']['value'])} minutes per week
• Expected Strength Gain: {monthly_gain:.1f} kg per month

DETAILED GUIDELINES:
• Rep Ranges: {'3-5 (strength focus)' if predictions['intensity']['value'] > 85 else '6-10 (hypertrophy)' if predictions['intensity']['value'] > 70 else '10-15 (endurance)'}
• Rest Between Sets: {'3-5 min' if predictions['intensity']['value'] > 85 else '90-120 sec' if predictions['intensity']['value'] > 70 else '60-90 sec'}
• Cardio Type: {'HIIT + LISS mix' if predictions['cardio_minutes']['value'] > 150 else 'Moderate LISS'}
• Cardio Sessions: {int(predictions['cardio_minutes']['value'] / 30)} sessions per week

================================================================================
3-MONTH STRENGTH PROJECTION
================================================================================
Expected Monthly Gain: {monthly_gain:.1f} kg on main lifts

Current → After 3 Months:
• Bench Press: {bench_press}kg → {bench_proj[-1]:.0f}kg (Δ{bench_proj[-1]-bench_press:.0f}kg)
• Squat: {squat}kg → {squat_proj[-1]:.0f}kg (Δ{squat_proj[-1]-squat:.0f}kg)
• Deadlift: {deadlift}kg → {deadlift_proj[-1]:.0f}kg (Δ{deadlift_proj[-1]-deadlift:.0f}kg)
• Total: {total_lifted}kg → {bench_proj[-1]+squat_proj[-1]+deadlift_proj[-1]:.0f}kg

================================================================================
NUTRITION TARGETS
================================================================================
• Protein: {protein_target:.0f}g per day (1.8g/kg)
• Caloric Approach: {'Surplus (+300-500 kcal)' if goal_muscle_gain else 'Deficit (-300-500 kcal)' if goal_fat_loss else 'Maintenance calories'}
...
MODELS USED
================================================================================
• Training Split: {models['training_split']['model_name']}
• Training Days: {models['training_days']['model_name']}
• Weekly Volume: {models['weekly_volume']['model_name']}
• Intensity: {models['intensity']['model_name']}
• Cardio Minutes: {models['cardio_minutes']['model_name']}
• Strength Gain: {models['expected_strength_gain']['model_name']}

All models optimized via hyperparameter tuning (Grid/Random Search CV)

================================================================================
Generated by AI Fitness Training Optimizer
Multi-Model System | 6 Specialized ML Models
Training Date: {model_package['training_date']}
================================================================================

DISCLAIMER: This is an AI-generated recommendation based on statistical models
trained on synthetic data. Always consult with qualified fitness and medical
professionals before starting any new training program.
"""
    
    st.download_button(
        label="📥 Download Complete Training Plan (TXT)",
        data=training_plan_text,
        file_name=f"training_plan_{pd.Timestamp.now().strftime('%Y%m%d')}.txt",
        mime="text/plain",
        use_container_width=True
    )

st.markdown("---")
st.caption("💪 AI Fitness Training Optimizer | Powered by 6 Specialized Machine Learning Models")
st.caption("⚠️ Disclaimer: AI-generated recommendations. Consult fitness professionals before starting any program.")
st.caption(f"🤖 System trained on {model_package['dataset_size']:,} samples | Last updated: {model_package['training_date']}")
